(function (global) {
  ...
  function summarize(entries, selfcare) {
    ...
    const breathingCount = selfcare.reduce((sum, day) => sum + (day.breathing?.sessions || []).length, 0);
    ...
    return {days: entries.length, mood: moodName(top), stepAverage, waterAverage, exerciseCount, breathingCount, hasDemo, allDemo};
  }
  ...
  function careTrend(summary) {
    ...
    const pause = summary.breathingCount ? "你也给自己留过安静呼吸的片刻" : "安静停下来的时刻还可以再多一点";
  ...
  function buildFallback(entries, selfcare = collectSelfCare(), options = {}) {
    ...
    if (demo) {
      ...
      content = `这周的心情有自己的起伏，记录里更多呈现出${summary.mood}的底色。你愿意把真实感受写下来，也在日常里慢慢照顾身体，这份认真值得肯定。${careTrend(summary)}。下周不妨保留一个固定的十分钟，喝水、走动，再做三次慢呼吸，给忙碌留一道缓冲。`;
    } else {
      content = `这一周的记录比较完整，心情虽然有变化，但整体呈现出${summary.mood}的底色。你持续为自己的感受留出位置，也在一些普通时刻做了照顾身体的选择，这种稳定的觉察很难得。${careTrend(summary)}，仍有几天可能被忙碌打乱。下周可以把目标收得更小：每天选一个固定时刻，先喝一杯水，再走动十分钟；如果心里拥挤，就留三分钟只关注呼吸。`;
    }
