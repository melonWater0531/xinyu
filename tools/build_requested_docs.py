...
def catalog_rows():
    ...
    add("手势交互", "Open Palm", "唤起心屿", "张手映射 summon_xinyu，启动短语音录入或聚焦聊天。", "保持 Open Palm 至少 4 帧；确认 intent_ready；查看语音/聊天反馈。", "已部署（资源待补）", "不控制云台；麦克风权限不可用时回退文字输入。", "只触发一次陪伴反馈，3 秒内不重复。")
    add("手势交互", "Closed Fist", "暂停/收起提醒", "握拳映射 pause_or_mute，单次稳定握拳立即响应。", "保持 Closed Fist；确认 intent_ready；观察语音 stop 和 toast。", "已部署（资源待补）", "不再要求二次握拳；不影响云台控制。", "当前语音播放/监听停止，提醒收起。")
    ...
    add("前端PWA", "预览", "Legacy Page 2 独立预览", "旧版视觉方案与情绪素材的静态预览，已从活跃 dashboard 归档。", "查看 archive/legacy_20260704/dashboard_preview/page2_preview/index.html；需要服务时按归档 README 恢复。", "已归档", "/home 当前由 dashboard/home.html 提供。", "legacy contract 测试保护数据和布局文案。")
    ...
    add("后端接口", "语音", "TTS 与 reCamera 扬声器播放", "智谱/OpenAI-compatible TTS 生成 WAV，FastAPI 缓存后经 Node-RED audio bridge 播放。", "配置 ZHIPU_API_KEY；部署 /audio/* bridge；POST /api/voice/tts play=true。", "已部署（设备播放待现场验收）", "TTS endpoint、声线和设备扬声器需现场验证；失败回退浏览器。", "audio_url 可访问，/api/voice/state.playback 记录 playing/done/error。")
    ...
    add("扩展规划", "专注", "停留时长专注指标", "后台聚合 face_visible_sec/focused_visible_sec/away_sec 并纳入 LLM 总结。", "运行 /home；GET /api/day_summary；调用 report 模式。", "已部署（后台聚合）", "不在前端显示秒表；需防止把在场等同于专注。", "日总结能自然提及陪伴趋势，不逐秒报数。")
