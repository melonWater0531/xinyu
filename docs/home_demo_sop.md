...
- 当前仓库只保留正式 `/home` 产品页和 `/control` 调试台，旧版 Home 与历史预览资产已清理。
...
- 会议页保留《下半年活动规划与预算申报周会》的本地会议样例；会议纪要未实现为可交付能力。
- 陪伴页使用今日状态作为小屿记忆上下文，回复会结合情绪趋势、喝水、步数、冥想、会议和日记。
- 页面优先连接真实接口；接口、设备或云端 key 不可用时使用本地记录和 seed data 兜底，不白屏。

推荐录屏时直接打开 `http://localhost:8001/home`：先展示首页当前情绪、趋势、今日状态与小屿建议，再依次切换陪伴、会议、记录和我的。会议页可以说明当前有本地会议样例和录音/ASR 工程链路，但不要表述为真实会议纪要已完成。

## 数据源说明

正式 `/home` 复用 `dashboard/product_home/seed_data.js` 中的本地产品体验数据，包括 2026 年 6 月 1 日到 7 月 4 日的记录、《下半年活动规划与预算申报周会》本地会议样例、日记、周报和小屿记忆上下文。页面会优先读取 `/ws` 或 `/api/state`，并接入聊天、日记、周报、会议和语音接口；失败时继续使用本地数据。
